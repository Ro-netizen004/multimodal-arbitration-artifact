...
The role-neutral experiment is the paper's primary analysis and is therefore the
default.  Pass ``--prompt-role original`` to reproduce the earlier text-designating
Phase 6/7 analysis.
"""
import argparse
import json
...
def paired_differences_by_id(model, benchmark, arm, prompt_role="neutral"):
    base = result_dir(model, benchmark, arm, prompt_role)
...
def paired_differences(model, benchmark, arm, prompt_role="neutral"):
    return np.asarray(
        list(
            paired_differences_by_id(
                model, benchmark, arm, prompt_role=prompt_role
            ).values()
        ),
        dtype=float,
    )
...
def paired_stats(differences, seed=20260720, n_resamples=10_000):
    ...
    # have either sign.  Use the median so the test, estimate, and bootstrap CI
    # target the same summary statistic.
    observed = abs(float(np.median(d)))
    extreme = 0
    batch = 2_000
    for start in range(0, n_resamples, batch):
        ...
        extreme += int(
            np.count_nonzero(np.abs(np.median(signs * d, axis=1)) >= observed)
        )
...
def main():
    ...
    parser.add_argument(
        ...
        default="neutral",
        help="Result tree to analyze. Neutral is the paper's primary analysis.",
    )
    ...
    for arm in ("image", "text"):
        ...
        for benchmark in ("gsm8k", "svamp"):
            ...
            for model in DISPLAY:
                result = paired_stats(
                    paired_differences(
                        model, benchmark, arm, prompt_role=args.prompt_role
                    ),
    ...
    for benchmark in ("gsm8k", "svamp"):
        print(f"  {benchmark}")
        for model in DISPLAY:
            image_change = paired_differences_by_id(
                model, benchmark, "image", prompt_role=args.prompt_role
            )
            text_change = paired_differences_by_id(
                model, benchmark, "text", prompt_role=args.prompt_role
            )
    ...
    for benchmark, model, r_image, r_text, result in latex_rows:
        ...
        previous = benchmark
    print("\\bottomrule\n\\end{tabular}")
    print("\\caption{Direct paired asymmetry contrast under the "
          f"{args.prompt_role.replace('_', '-')} prompt. $R_I$ and $R_T$ are median "
