...
BENCHMARK_REGISTRY = {
    ...
    "gsm8k":    {"loader": load_gsm8k,       "type": "text_math",       "has_images": False,
                 "hf_repo_id": "[ANONYMIZED_DATASET_NAMESPACE]/gsm8k-rendered-vlm-v2"},
    "svamp":    {"loader": load_svamp,        "type": "text_math",       "has_images": False,
                 "hf_repo_id": "[ANONYMIZED_DATASET_NAMESPACE]/svamp-rendered-vlm-v1"},
    "aqua_rat": {"loader": load_aqua_rat,     "type": "text_math",       "has_images": False,
                 "hf_repo_id": "[ANONYMIZED_DATASET_NAMESPACE]/aqua-rat-rendered-vlm-v1"},
    "math":     {"loader": load_math_dataset, "type": "text_math",       "has_images": False,
                 "hf_repo_id": "[ANONYMIZED_DATASET_NAMESPACE]/math-rendered-vlm-v1"},
...
def load_benchmark_from_hf(name: str, num_problems: int = None) -> List[BenchmarkItem]:
    ...
    # Anonymous artifacts must not publish the real dataset owner. Permit an
    # exact per-benchmark repository override, or replace only the redacted
    # namespace at runtime. Examples:
    #   VLM_GSM8K_RENDERED_REPO=anonymous-owner/gsm8k-rendered-vlm-v2
    #   VLM_DATASET_NAMESPACE=anonymous-owner
    env_key = f"VLM_{name.upper()}_RENDERED_REPO"
    explicit_repo = (
        os.environ.get(env_key)
        or os.environ.get("VLM_ARITHMETIC_DATASET_REPO")
    )
    if explicit_repo:
        repo_id = explicit_repo
    elif repo_id.startswith("[ANONYMIZED_DATASET_NAMESPACE]/"):
        namespace = os.environ.get("VLM_DATASET_NAMESPACE")
        if not namespace:
            raise RuntimeError(
                f"The artifact redacts the Hugging Face owner for {name}. Set "
                f"{env_key} (or VLM_ARITHMETIC_DATASET_REPO) to the full "
                "rendered or combined arithmetic dataset ID, or set "
                "VLM_DATASET_NAMESPACE to its owner/namespace."
            )
        repo_id = f"{namespace.rstrip('/')}/{repo_id.split('/', 1)[1]}"

    ds = load_dataset(repo_id, split="train")

    # The released combined arbitration dataset can stand in for the separate
    # rendered benchmark repositories.  Its clean image-arm rows contain the
    # same canonical image, question, answer, and problem ordering required by
    # the runner; later levels/arms are regenerated by run_legibility.py.
    combined_fields = {
        "source", "channel", "degradation_level", "image_problem_id",
        "image_question", "image_answer", "image",
    }
    combined_schema = combined_fields.issubset(set(ds.column_names))
    if combined_schema:
        ds = ds.filter(
            lambda row: row["source"] == name
            and row["channel"] == "image"
            and int(row["degradation_level"]) == 0
        ).sort("image_problem_id")
        if not len(ds):
            raise RuntimeError(
                f"Combined dataset {repo_id!r} has no clean image-arm rows for {name}."
            )
    ...
    for row in ds:
        ...
        answer = str(row.get("image_answer" if combined_schema else "answer", ""))
        ref_num = extract_number_from_answer(answer)
        ...
        items.append(BenchmarkItem(
            id=row.get("image_problem_id" if combined_schema else "problem_id", len(items)),
            question=row.get("image_question" if combined_schema else "question", ""),
            reference_answer=answer,
            reference_number=ref_num,
            image=image,
            choices=choices,
